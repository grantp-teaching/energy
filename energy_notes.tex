...
The PUE is the main metric of energy efficiency in a data centre environment: 
\begin{align}
  \PUE & = \frac{\mbox{total power input to the data centre}}{\mbox{total power input to IT equipment}} = 1 + \frac{\mbox{non-IT power input}}{\mbox{IT power input}}
\end{align}
The minimum value of the PUE is theoretically 1.
Ideally we want to reduce the PUE to as low a figure as possible. 
...
Reducing energy costs requires a consistent across-the board approach to reducing both the IT and non-IT power loads, \autoref{fig:options-to-reduce-energy-consumption}.

\begin{figure}[htbp]
  \centering
  \includegraphics{energy_reductions}
  \caption{Options to reduce energy consumption}
  \label{fig:options-to-reduce-energy-consumption}
\end{figure}

Best to take advantage of easy wins, avoid diminishing returns.
...
\begin{figure}[htbp]
  \centering
  \includegraphics{containment_photo_subzero}
...
\begin{figure}[htbp]
  \centering
  \includegraphics{free_cooling_europe_map_systemair}
...
\begin{figure}[htbp]
  \centering
  \includegraphics{glycool_liebert}
